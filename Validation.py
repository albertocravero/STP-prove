...
path = '/Users/giancra/Desktop/IA/HummingBird 3.0/HB/open/60centers lr: w_0.01 ∂_0.001 0try new formulation/model'
model = torch.load(path)
with torch.no_grad():
    agent.A_val.sigma = model['sigma']
    agent.A_val.weights = model['weights']
    if len(model)>3:
        agent.A_val.centers = model['centers']
...
for epoch in range(num_epochs):
    ...
    state = env.reset()
    state = env.reset_validation([-0.7,-0.6])
    done = False
    actions = []
    init_dist.append(state[0])
    ...
    if episode_loss < best_model['loss']:
        ...
        best_model['model'] = {'weights': agent.A_val.weights,'sigma': agent.A_val.sigma,'centers': agent.A_val.centers}
...
plt.figure(8)
plt.plot(best_model['drag'], label='drag')
plt.legend(loc='best')
